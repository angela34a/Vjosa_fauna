...
spt_vj <- read_excel("Spatial_vjosa_v3.xlsx") %>% 
          as.data.frame() %>% 
          column_to_rownames("Code") %>% 
...
#...check for ~ Site and for shasnnons index and so on ... 
...
```{r}
  ...
  ggplot( as.data.frame(scores(simpleRDA)$sites), 
          aes(x=RDA1,y=RDA2)) +
...
# plot
  ggplot(aes(x = River, y = rel_abund, fill = species)) + 
